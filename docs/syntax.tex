...
\begin{figure}[H]
  ...
  \begin{tikzpicture}
    ...
    \node[nterm] (id) [below=of record] {Right Identifier};
    \node[nterm] (chain) [below=of id] {Morph Chain};
    \coordinate [right=of record] (choices-exit);
    \node[exit] (exit) [right=of choices-exit] {};

    \draw[narr, mid arrow = 0.75] (entry) -- (record);
    \draw[narr, mid arrow = 0.25] (record) -- (exit);

    \draw[narr, mid arrow = 0.7] (entry) -- (entry-choices) |- (id);
    \draw[narr, mid arrow = 0.3] (id) -| (choices-exit) -- (exit);

    \draw[narr, mid arrow = 0.85] (entry) -- (entry-choices) |- (chain);
    \draw[narr, mid arrow = 0.15] (chain) -| (choices-exit) -- (exit);
  \end{tikzpicture}
\end{figure}
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}
    ...
    \coordinate[below=1.25cm of mid] (bot);
    ...
    \node[nterm] (id) [below=0.25cm of top] {Identifier};
    ...
    \draw [arr] (entry-loop) -| (bot-right-right) -- +(-0.85, 0);
